...
// CRM 9 version with form context.
QALab.OnFormLoad = function (context) {
    var xrmpage = context.getFormContext();
    QALab.LockAccountNumber();
    QALab.addOnChange(xrmpage,"new_sortcode", QALab.CheckLocationAgainstSortCode);
    QALab.addOnChange(xrmpage,"new_banklocation", QALab.CheckLocationAgainstSortCode);
    QALab.addOnChange(xrmpage, "new_balance", QALab.CheckFundsForAccountType);
    QALab.addOnChange(xrmpage, "new_accounttype", QALab.CheckFundsForAccountType);
};

QALab.addOnChange = function (xrmpage, attribute, callback) {
    var attr = xrmpage.getAttribute(attribute);
    if (attr != null) attr.addOnChange(callback);
}
